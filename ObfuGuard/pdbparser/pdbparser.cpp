...
pdbparser::pdbparser(pe64* pe_image) {
    ...
    if (dbg_dir_va + sizeof(IMAGE_DEBUG_DIRECTORY) > buffer_size) {
        SymCleanup(process_handle);
        throw std::runtime_error("Debug directory virtual address is out of bounds.");
    }
    ...
    for (DWORD entry_idx = 0; entry_idx < max_entries && dbg_dir_ptr->SizeOfData; ++entry_idx, ++dbg_dir_ptr) {
        if (dbg_dir_ptr->Type != IMAGE_DEBUG_TYPE_CODEVIEW) {
            continue;
        }

        if (dbg_dir_ptr->PointerToRawData + dbg_dir_ptr->SizeOfData > pe_image->get_buffer_not_relocated()->size()) {
            continue;
        }

...
private:
    ...
    bool get_and_relocate_original_function_code(
        ...
        size_t known_function_size = 0
    );

    bool create_new_section(const std::string& section_name, uint32_t initial_size = ObfuGuard::DEFAULT_SECTION_SIZE);
...
public:
    ...
    [[nodiscard]] bool inject_function_trampoline(uint32_t function_rva, uint32_t function_size = 0);
...
// High-level junk-code injection modes (uses ObfuGuard::function_filter for blacklists)

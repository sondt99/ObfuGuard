...
// Safebufer để điều chỉnh entry point phù hợp với alignment sau khi vá
...
// tìm lệnh theo vị trí runtime
bool obfuscatecff::find_inst_at_dst(uint64_t dst, instruction_t** instptr, function_t** funcptr) {
...
// lựa chọn và áp dụng các thuật toán làm rối mã
void obfuscatecff::run(PIMAGE_SECTION_HEADER new_section, bool obfuscate_entry_point) {
...
// tổng bộ nhớ đã obfu
uint32_t obfuscatecff::get_added_size() {
	return this->total_size_used;
}


// dịch các câu lệnh hợp ngữ từ biên dịch just-in-time
std::vector<obfuscatecff::instruction_t>obfuscatecff::instructions_from_jit(uint8_t* code, uint32_t size) {
...
// detect các lệnh jmp của từng câu lệnh
...
// lấy thông tin về các giá trị tham chiếu tương đối
void obfuscatecff::instruction_t::load_relative_info() {
...
void obfuscatecff::instruction_t::load(int funcid, std::vector<uint8_t>raw_data) {
...
void obfuscatecff::instruction_t::load(int funcid, ZydisDisassembledInstruction zyinstruction, uint64_t runtime_address) {
	this->inst_id = instruction_id++;
...
void obfuscatecff::instruction_t::reload() {
...
void obfuscatecff::instruction_t::print() {
	char buffer[256];
